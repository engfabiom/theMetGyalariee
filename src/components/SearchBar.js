...
function SearchBar({ setTheMetObject, onSearch }) {
  ...
  function handleCheck(event) {
    ...
    // console.log({ ...areChecked, [name]: checked });
  }
  ...
  async function newQ() {
    ...
    const query = userQ || "*";
    const toQuery = await apiSearch(query,isHighlight,isOnView);
    setUserQ("");
    setAreChecked({isHighlight: false,isOnView: false});
    setIsMenuOpen(false);
    console.log(toQuery);
    if (onSearch) {
      onSearch(toQuery);
    } else {
      console.log("onNewSearch doesnt exist");
    }
  }
